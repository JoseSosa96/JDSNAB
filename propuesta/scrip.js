...
var ganadosHumano = 0;
var propuesta = "¿TE CASARIAS CONMIGO?"
const array = ['  ',' \xbf - CON ', ' RI - TE ', ' GO - SA ', ' MI - AS ', ' CA - ? '];
//ids de las celdas en el html
var IDS = [
  ["ceroCero", "ceroUno", "ceroDos"],
...
function jugadaComputadora(tateti) {
  ...
  if (!tateti.estaTerminado()) {
    ...
    var posiblesParaGanar = tateti.celdasVaciasDeLineasConDosOcupadas(
      tateti.fichaComputadora
    );
    var posiblesParaBloquear = tateti.celdasVaciasDeLineasConDosOcupadas(
      tateti.fichaHumano
    );

    if (posiblesParaGanar.length >= 1) {
      posicion =
        posiblesParaGanar[Math.floor(Math.random() * posiblesParaGanar.length)]; //elijo al azar una de las celdas
    } else if (posiblesParaBloquear.length >= 1) {
      posicion =
        posiblesParaBloquear[
...
function mostrarMarcador() {
  ...
  letras+= " "+ array[ganadosHumano];
  if(ganadosHumano==1){
    letras=array;
    setTimeout("location.href = 'src/INDEX.html#';", 3000);
  }
  // window.location.href = "file:///C:/Users/Mi%20PC/Desktop/Nueva%20carpeta/INDEX.html#";
  ganadosC.textContent = "Letras obtenidas: " + letras;
  ganadosH.textContent = "Nati: " + ganadosHumano;
  empate.textContent = "Empates: " + empates;
...
function eligeFicha(tipoFicha) {
  ...
  hum.textContent = "Nati juega con " + tateti.fichaHumano;
  comp.textContent = "PC juega con " + tateti.fichaComputadora;
